...
def k_means(k_means_array, k, image_as_array, num_of_rows, num_of_cols):
    ...
    for i in range(0, num_of_rows * num_of_cols):
        potential_short_distance_between_points = distance(k_means_array[0][0], k_means_array[0][1],
                                                           k_means_array[0][2],
        ...
        for j in range(1, k):
            potential_short_distance_between_points2 = distance(k_means_array[j][0], k_means_array[j][1],
                                                                k_means_array[j][2],
                                                                image_as_array[int(i / num_of_cols)][i % k - 1][0],
                                                                image_as_array[int(i / num_of_cols)][i % k - 1][1],
                                                                image_as_array[int(i / num_of_cols)][i % k - 1][2])
...
def min_distance_from_point_to_mean():
    distance_between_point_and_mean = distance()
